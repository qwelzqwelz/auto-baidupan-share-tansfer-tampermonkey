...
!(function () {
    ...
    obj.basicQueryItem = function (selector, nid) {
        var $this = $(selector);
        $.get(
            "https://tb.idey.cn/taobao.php?act=itemlink&itemid=" + nid,
            function (data) {
    ...
    obj.get_type_url = function (url) {
        if (
            ...
            return "jd_item";
        } else if (url.indexOf("//miaosha.jd.com") > 0) {
            return "jd_miaosha";
        } else if (url.indexOf("//www.jd.com") > 0 || url.indexOf("//jd.com") > 0) {
            return "jd_index";
        } else if (url.indexOf("//pan.baidu.com") > 0 || url.indexOf("//yun.baidu.com") > 0) {
            return "baidu_pan";
        } else if (url.indexOf("//jingfen.jd.com") > 0) {
            return "jingfen";
        }
    };
    ...
    if (pagetype == "taobao_item") {
        ...
        //	var productId = obj.get_page_url_id(pagetype, pageurl, pageurl);
        var couponurl =
            "https://s.idey.cn/ltb.php?act=items&itemurl=" + encodeURIComponent(location.href) + "&itemid=" + productId;
        $.getJSON(couponurl, function (res) {
            ...
            couponArea += '<span class="blkcolor1"><span style="color:red" id="now_price">每日捡漏微群</span></span>';
            ...
            couponArea += " </div></div>";
            couponArea += ' <div class="idey-mini-placeholder idey-price-protect"></div><div id="promo_box"></div>';
            ...
            if (res.type == "success") {
                ...
                couponArea +=
                    ' <div class="coupon-tle"> <span>当前商品领券立减' +
                    data.couponAmount +
                    '元</span> <em class="coupon_gif"></em></div>';
                couponArea +=
                    '<div class="click2get"><span class="c2g-sp1">￥' +
                    data.couponAmount +
                    '</span><span class="c2g-sp2">领取</span></div>';
                couponArea += "</a>";
            } else {
                couponArea += '<a id="coupon_box" title="" class="coupon-box1" >';
                couponArea += '<span class="coupon-icon"></span>';
        ...
        var couponurl =
            "https://tbao.idey.cn/jd.php?act=recovelink&itemurl=" +
            encodeURIComponent(location.href) +
            "&itemid=" +
            productId;
        $.getJSON(couponurl, function (res) {
        ...
        var couponurls =
            "https://s.idey.cn/xjd.php?act=item&itemurl=" + encodeURIComponent(location.href) + "&itemid=" + productId;
        ...
        $.getJSON(couponurls, function (res) {
            ...
            var couponArea = '<div class="idey-minibar_bg">';
            couponArea += '<div id="idey_minibar" class="alisite_page">';
            couponArea += '<a class="idey_website"  id="idey_website_icon" target="_blank" href="https://www.idey.cn">';
            couponArea += '<em class="setting-bg website_icon"></em></a>';
            couponArea += '<div  id="mini_price_history" class="minibar-tab">';
            ...
            if (res.type == "success") {
                if (data.couponLinkType == 1) {
                    couponArea += '<a id="coupon_box" title="" class="coupon-box1" href="' + data.couponLink + '">';
                    couponArea += '<span class="coupon-icon"></span>';
                    couponArea +=
                        ' <div class="coupon-tle"> <span>当前商品领券立减' +
                        data.couponAmount +
